...
# We assume app.py is in backend/ and frontend/ is at ../frontend relative to it.
...
# Load soil averages
try:
    soil_df = pd.read_csv("data/soil_averages.csv")
    soil_df['district'] = soil_df['district'].str.lower().str.strip()
    soil_averages = soil_df.set_index('district').to_dict('index')
except Exception as e:
    print(f"Warning: Could not load soil averages: {e}")
    soil_averages = {}
...
def transcribe_proxy():
    ...
    try:
        # Deepgram expects the raw audio data
...
def get_soil_profile():
    ...
    # Try direct match
    profile = soil_averages.get(district)
    
    # If not found, try partial match
    if not profile:
        for d, p in soil_averages.items():
    ...
    # Fallback to default
    if not profile:
        profile = soil_averages.get('default', {"N": 130, "P": 45, "K": 170, "ph": 7.0})
